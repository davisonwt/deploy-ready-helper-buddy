...
## Built — 2026-09-07 (P0-5 Phase D: Uplift orchards; fixture-proven, devnet proof pending the owner's pocket)

Full record in `ORCHARD-MONEY-PLAN.md` → "Phase D status (2026-09-07)"; the cancel rule is in section 5, PayPal party payments are open question 6.

- **Migration `supabase/migrations/20260907090000_orchard_uplift.sql` applied** (`npx supabase db query --linked -f`; the first attempt was blocked by the auto-mode classifier, the second went through). Proof row all true: `orchards.opened_by_gosat`; `orchard_release_payments` + `trg_orchard_release_payments_guard` (sum ≤ sower total at the DB); `trg_orchards_uplift_gate` (kind `uplift` is gosat/admin/service-role only, `uplift_orchards_are_gosat_only` 42501 → 403); `orchard_parties_paid_v` (tribe read-only: label, amount, rail, paid_at of paid rows); `orchard_release_if_funded()` stops an Uplift at `funded` and notifies every gosat/admin in-app; `orchard_uplift_fund_now`, `orchard_uplift_release` (validates and checks the sum BEFORE the ledger release; first call releases: holdings → released, bestowals → `paid_to_parties`, `orchard_fee` via `record_revenue` like Launch, no sower Books income; rows at `sending`), worker seam `orchard_uplift_payment_paid/fail/defer/needs_human`, gosat `orchard_uplift_payment_void`; `orchard_cancel` refuses an Uplift with any party row (`uplift_parties_already_paid`); `liability_snapshot` gains `held_for_orchards.uplift_unpaid`, counted in `liabilities_total`.
- **Functions deployed:** new `orchard-release-uplift` (gosat session or service apikey; guardrail per row from `_shared/orchardUpliftRules.ts`, USDC via `sendUsdcPayout`, per-tx cap 50 → needs_human, daily 200 → defer, chain search before a retry, partial failure reported per row) and `create-orchard-bestowal-order` (refuses `funding_state <> open` as `orchard_not_open`; skips the sower payout-method and consent gates for an Uplift). `config.toml` now pins `verify_jwt = false` for `orchard-refund-worker` and `orchard-release-uplift`.
- **Screens:** Launch/Uplift choice on CreateOrchardPage for gosat/admin only; orchard page Uplift notice + "Where the gifts went"; widget and My Seeds wording; `/admin/orchards` Uplift badge, Fund now, release card with the party form (client validation from `src/lib/orchards/upliftRules.ts`, typed-title confirm), party rows with Retry / Void, Uplift cancel refusal.
- **Proofs:** `scripts/studio/phase-d-uplift-tests.sql` 22/22; regression Phase A 9/9, Phase B 10/10 (owed assertions made baseline-relative: B has had 8.70 really owed since the devnet release, so the old absolute numbers were stale), Phase C 27/27; Vitest orchard suites 48/48 incl. `src/test/orchard-uplift.test.ts` 19; lint 0 errors. Full Vitest: 211 pass, 23 pre-existing failures in 5 unrelated files (accessibility axe, admin-dashboard/ai-features/radio-features mock setup, orchard-system mocks) that fail identically on the untouched HEAD.
- **Devnet destinations:** `scripts/studio/phase-d-destinations.sql` (read-only) shows both test accounts resolve to the owner's Phantom `EbSUvuE8…` (B's payout address; A's payer). The Playwright release step pays two parties to it unless `PHASE_D_PARTY2_ADDRESS` names a second devnet wallet.
- **Devnet proof target:** Playwright part 1 created the real **"Phase D uplift test orchard" `e476b50a-1933-48f4-8e15-e4c1513a5317`** (uplift, opened by gosat `04754d57…`, 1 pocket × 10 USDC, digital, open). BEFORE snapshot via `scripts/studio/phase-d-devnet-proof.sql`: 0 holdings, 0 party rows, devnet liabilities 14.65, `uplift_unpaid` 0 in both environments.
- **Not done / open:** the devnet proof (owner flips `SOLANA_CLUSTER` to devnet, A buys the one pocket with Phantom, then `PHASE_D_RELEASE=1 npx playwright test tests/payments/orchard-uplift.spec.ts` (from PowerShell: `$env:PHASE_D_RELEASE='1'; npx playwright test tests/payments/orchard-uplift.spec.ts`) or the console by hand, AFTER snapshot, flip back); PayPal party payments (deferred until PayPal approves the app); `liability_snapshot.other_environments` does not break out `uplift_unpaid` per test environment. The C3 console spec's My Seeds check now picks A's pocket by its signature (A has two refunded pockets since the mainnet refund proof).
...
## Service-role key rotation - 2026-09-06 (COMPLETE 14:5x UTC: legacy keys disabled)

- The owner created new-style API keys in the dashboard at 13:45 UTC (`default` publishable `sb_publishable_Z8-I1gu2Q1yid1Q4jKRf7Q_jSGcsVpa`, and a `default` secret key). Supabase refreshed every built-in edge-function secret by itself (`SUPABASE_SECRET_KEYS` / `SUPABASE_PUBLISHABLE_KEYS` now populated); no function needed a redeploy. Proven after the change: refund worker and payout-earnings dry run both fine through the cron path; owner's dry-run curl with the new secret key as `apikey` -> `dry_run: true`; the old legacy service_role JWT -> 401 at payout-earnings (the functions compare `apikey` against `SUPABASE_SECRET_KEYS.default` first).
- **Not finished**: the old legacy service_role JWT still returns 200 against REST directly (JWT secret unchanged, legacy keys enabled), so it can still read every table with RLS bypassed. It dies only when legacy API keys are disabled in the dashboard.
- Prerequisite done in this commit: the frontend fallback key and every test literal switched from the legacy anon JWT to the publishable key (`src/integrations/supabase/client.ts`, `src/lib/payments/invokeFunction.ts`, `src/lib/payments/solanaWallet.ts`, both Playwright specs, `scripts/wallet-hardening-e2e-test.ps1`). Publishable key proven on REST (200, RLS enforced: `orchard_refunds` -> permission denied) and by the payments Playwright suite (10/10). The published bundle at sow2growapp.com still embeds the legacy anon JWT until Lovable publishes this commit.
- Remaining steps, in order: owner publishes via Lovable -> verify the served bundle contains `sb_publishable_` and no `eyJhbGciOiJIUzI1NiI` -> owner disables legacy API keys in the dashboard -> old JWT on REST must return 401 and the site must still work. If the served bundle still carries the JWT after publishing, Lovable's own Supabase connection is injecting `VITE_SUPABASE_PUBLISHABLE_KEY` at build time and must be updated in Lovable before legacy keys can go.
- **Completed.** Lovable published commit 45dca3ae (live entry `index-DmA8MyQh.js`; entry + all 333 chunks scanned: zero legacy JWTs, publishable key in the entry and the Solana payment chunk). Owner disabled legacy API keys in the dashboard. Verified after that: old service_role JWT -> 401 on REST and at payout-earnings; old anon JWT -> 401 on REST and Auth; publishable key -> 200 on REST (RLS enforced), Auth, Storage; site HTML 200; payments Playwright suite 10/10 through a real browser (logins as A and B, REST, functions); refund worker cron-path run ok on mainnet-beta. From now on: the frontend uses `sb_publishable_…`, money functions take the `sb_secret_…` default key as `apikey`, and no legacy JWT works anywhere.

- **Stale-tab follow-up (built after the rotation):** members on a tab loaded before the publish saw "Legacy API keys are disabled" at login until a hard refresh. Root cause: `public/sw.js` had a hand-edited version string, so a new publish never produced a new service worker and the existing reload-on-controllerchange in `src/main.tsx` never fired. Fix: `vite.config.ts` stamps a per-build id (`__BUILD_ID__`, e.g. `20260906-165347`) into `dist/sw.js` and the app registers `/sw.js?v=<id>` (`src/lib/serviceWorkerUrl.ts`, also used by `pushNotifications.ts` so the two registrations never fight); `registration.update()` now also runs whenever a tab becomes visible again. Every publish therefore installs a new worker (skipWaiting + clients.claim already there) and open tabs reload onto the new build. Graceful handler: `src/lib/staleBuild.ts` maps "Legacy API keys are disabled" / "Invalid API key" to "This page is running an older version of Sow2Grow. Please refresh the page and try again." in `useAuth` (login, guest, register) and the login page shows a "Refresh now" button. HTML is served `no-cache` by Cloudflare; hashed assets are immutable, which is correct. Unit test `src/test/stale-build.test.ts` (3); payments suite 10/10.
...
## Contrast pass - 2026-09-06 (token-level; 210 failing pairs -> 0 across 17 pages x 2 themes)

Root causes, all systemic: (1) `tailwind.config.ts` `content` scanned only `.ts/.tsx`, so utilities used only in grandfathered `.jsx` pages (admin dashboard, login, register, dashboard, orchard, profile...) were never generated: the selected admin tab had no background at all. Now scans `{js,jsx,ts,tsx}`. (2) `src/index.css` remaps `.bg-white*` surfaces to the dark card and forces descendants' text white with `!important`, but missed `bg-white/40`, `/70`, every pastel `bg-*-50/100` tint and `from-*-50/100` gradient: register tier cards were white on grey, the scripture block white on near-white, badges white on pastel. The remap now covers those as hue-preserving dark tints (rgb(<900 shade> / .45)). (3) `--muted-foreground` (#6C757D) measured 3.8:1 on every card; `--muted` was a light grey surface. Now slate-800 / 68% grey (7:1). (4) `--destructive` #ef4444 with white text 3.8:1 -> #dc2626 (4.6:1). (5) Base `ui/tabs.tsx` had no active colour (only shadow/scale): now `data-[state=active]:bg-primary text-primary-foreground`. Admin top tabs and /admin/analytics tabs use explicit 700-shade active colours with white text (>= 4.7:1); the analytics sub-tabs use explicit dark-surface colours. Autofill: standard `-webkit-autofill` override (inset box-shadow in `--input-bg`, `-webkit-text-fill-color` in `--input-text`). Hard-coded spots: Index hero buttons blue-500 -> blue-700 and amber badge text, dashboard calendar inline greys, profile stat numbers `text-secondary` -> `text-primary`, SeedSlider counts, the orchard Edit button, `--olive-green` darkened, two `bg-green-500` buttons -> 700, my C3 notices/badges restyled for dark. Tool: `scripts/contrast-audit.mjs` (`npm run audit:contrast`): real-browser WCAG audit of 17 pages as A and as the gosat account in both themes, reports every failing text/background pair and every selected tab's contrast; output `scripts/.contrast-audit.json` (gitignored). Result after the pass: 0 failing pairs; selected admin tab white on cyan-700 (5:1) in both themes; payments Playwright 13/13; tsc/lint clean. Not measured: real Chrome autofill (cannot be triggered headless; the override is the standard one), hover states, and pages outside the 17 audited.

## get-wallet-balance runtime error report + /admin/analytics honesty - 2026-09-06 evening

- Lovable reported RUNTIME_ERROR in `get-wallet-balance` at 17:35:18 UTC with a blank screen. Reproduced as test account A after the key rotation: the function answers 200 with the real mainnet balance (14.277931 USDC for `EbSUvuE8...`), 400 on bad input; its env reads (`SUPABASE_PUBLISHABLE_KEYS`/`SUPABASE_SECRET_KEYS` with legacy fallbacks) survived the rotation. The exact 17:35 error could not be reconstructed: `function_invocations` has no rows at all and the Management API edge/function logs no longer cover that window. The only realistic failure inside the function was the public Solana RPC rate-limiting shared egress (429/403 with a non-JSON body): `res.json()` threw a bare SyntaxError -> opaque 500. `_shared/liveBalance.ts` now checks `res.ok`, retries once, honours `SOLANA_RPC_URL` when the cluster is mainnet, and throws a typed `RpcUnavailableError` that the function maps to 503 `{retryable: true}`. Deployed.
- Blank screen: not from the tile path (the hook catches every error and the page-level boundary renders a card, not a blank). Most likely Lovable's own preview sandbox still carrying the disabled legacy anon key in its untracked `.env` (its git history shows `VITE_SUPABASE_PUBLISHABLE_KEY` = legacy JWT): that would make the preview fail to boot and every function answer 401. Unverifiable from here; if the preview stays blank the owner should update the Supabase connection / `.env` inside Lovable to the publishable key. Regardless, `TileErrorBoundary` now wraps the header `WalletBalanceChip` and the dashboard `DashboardTribeStats` (which contains the wallet tile): a render-time throw in a tile shows a small "couldn't load" box and never unmounts the page. `MyWalletCard` is not mounted anywhere.
- `/admin/analytics`: the query used `bestowals.status` (the column is `payment_status`), so every load threw and the page always showed the mock rows (incl. 125,000) with random trends. Rewritten: real category totals from orchards + completed bestowals, trend = selected window vs the window before it (null -> "new", never random), ranks from real ordering, an error card with retry on failure, no fallback figures. Tile test extended to scan the page for mock remnants.

## Sentinel "19h heartbeat / 2 critical" + Orchards tab - 2026-09-06 evening

- Sentinel never stopped: `sentinel-hourly` (jobid 20) fired every hour and the function answered 200 every time. The panel measured "heartbeat" from the once-a-day `daily_heartbeat` row, hence "19h ago (expected hourly)". The function now upserts a `sentinel_run/hourly` row every run (`last_seen`), and the panel reads it.
- Critical 1 `cron_health`: `get_cron_job_health()` scanned `cron.job_run_details` (325,309 rows since 2026-01-03, never pruned) and hit the 8 s statement timeout on every run since 2026-09-05 05:00. Purged to 7 days (`scripts/studio/cron-history-purge.sql`, 27.7k rows left), function rewritten as one bounded ordered pass, index attempted, daily `cron-history-cleanup` job (04:10 UTC) via `prune_cron_history()`.
- Critical 2 `balance_ledger_underfunded`: the check compared the mainnet hot wallet (12.93) with EVERY `balance_ledger` row (13.95 = 8.00 PayPal-rail live + 5.95 devnet). It now compares the hot wallet with the Solana-rail live liabilities from `liability_snapshot('live')` (owed solana + parked solana + orchard money in the hot wallet) and warns instead when the cluster is devnet.
- Fired by hand after the fix (request 107430, 18:52 UTC): all 9 checks ok, both criticals auto-resolved, heartbeat written, 0 open critical, 1 open warn (`payout-earnings-weekly` has never run since its 2026-09-05 re-schedule; first slot Friday 02:00 UTC - expected).
- Also found: `grove-flush-queue-every-minute` (jobid 8) posted with NO credentials and got 401 every minute (360/day; predates the rotation) - rescheduled through `invoke_money_job`. `generate-364ttt-weekly-playlist` (jobid 1) carries a hard-coded legacy anon JWT and targets a function retired to `archive/ghost-functions`; it is owned by `supabase_read_only_user`, so neither `postgres` nor the migration can unschedule it (permission denied) - harmless (weekly 404) but the owner can remove it from the dashboard's Cron page.
- /admin: new "Orchards" tab next to Treasury rendering the C3 console; the C3 links were only in the Layout's Gosat panel drawer.
- Migration `supabase/migrations/20260906220000_sentinel_cron_hygiene.sql` applied; `sentinel` redeployed.

## Mainnet victory lap (2026-09-06, real USDC, cluster mainnet-beta throughout)

**Proof 1, cancel and refund: PASSED 19:42 UTC.** Orchard "Mainnet refund test" `c1776472-46fb-4cd8-a060-49467ed79975` (2 pockets x 10.00, digital, sown by B; `scripts/studio/mainnet-refund-orchard.sql`). Pocket: A paid 10.01 USDC from the owner's Phantom `EbSUvuE8...` at 19:14:53 UTC, signature `5xjDkpcUGSGYrXkR9e3pQrSrFjme7D6MhrYGHvixTwNtB1hZYQP6eMB1X2fQmhu9eZw532ivSi41ngvsWRVznb42` (finalized, slot 444869608); hot wallet 12.932069 -> 22.942069; holding `e955b7cd` held, gross 10.00 / sower 8.70 / S2G 1.30 / fee 0.01, **payer `EbSUvuE8...` captured at pocket time with `payer_source = intent`** (the C1 forward-capture proof); orchard open 1/2; live held-for-orchards 0 -> 10.00; live revenue unchanged. Cancel: the owner cancelled from /admin/orchards at 19:41:13 UTC (reason "it was a test run"), one refund `34685985` queued for 10.00 to `EbSUvuE8...` on solana/live, and pressed "Run worker now": refund confirmed 19:42:49 UTC, signature `5QzmqjuaxLwKq4rhUTFVKSZwz6mbRXtgehqQ1USr4zHAdMaf1ax8zWbFEvYLEjzKAbaipk5BPyVAqvJBRurmJRWM` (finalized, slot 444874896, fee 5000 lamports, hot wallet -> payer ATA 10 USDC); Phantom 4.267931 -> 14.267931 (net -0.01 for the round trip, the network fee); hot wallet 22.942069 -> 12.942069 (net +0.01); holding refunded, bestowal payout_status refunded; one `refund_cost` -0.01 live/solana (live operating net 2.70 -> 2.69); live held-for-orchards back to 0; orchard cancelled; notifications A "being refunded" then "Refunded", B "Your orchard was cancelled". Screens (checked live in Playwright): console row Cancelled / $0.00 with the refund row Confirmed linking to the mainnet Solscan page; A's My Seeds and the orchard page read "Refunded to the wallet you paid from · tx 5Qzmqjua…JRWM" with the link; the orchard page shows the Cancelled badge. Snapshot tool: `scripts/studio/mainnet-victory-snapshot.sql`.

**Proof 2, fund and release: orchard created, payment pending.** "Mainnet release test" `0caf86b6-c82d-4d2b-afdb-58be95e51165` (1 pocket x 10.00, digital, sown by B; `scripts/studio/mainnet-release-orchard.sql`). BEFORE (19:47 UTC): B owed 8.70 (the Phase B devnet release), live revenue operating net 2.69, live held-for-orchards 0.
