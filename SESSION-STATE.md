...
**New problem, found by this real transaction, more urgent than the above:** the buyer cannot access what they paid for. `product_bestowals` says `completed`/`released`, but `MusicTrackDetailPage.tsx`'s `resolveMediaUrl()` can only create a signed URL on the `premium-room` bucket for the track's own uploader — a pre-existing, already-documented RLS gap (Open item #10) — so `audioUrl` resolves to `null` for the actual buyer, and the UI has no download affordance at all for a product-sourced (non-`dj_track`) owned track. The buyer's actual screen: "Preview isn't available for this seed yet." plus "✓ Bestowed — thank you for supporting this sower," with no way to play or download the file. Confirmed live: `GET` on the stored `file_url` directly returns `400` (the bucket is genuinely private, not the misleading `/object/public/...` shape stored in `products.file_url` would suggest).
