...
8. **`S2GCommunityMusicPage.tsx`'s 4th duplicate `isAlbum()` check** — flagged during consolidation, still not fixed. The file has since been touched for unrelated payment-picker/pay-currency work (`9d1a9b7d`, `4300dabb`) — the `isAlbum()` duplicate itself was not part of that and remains outstanding.
