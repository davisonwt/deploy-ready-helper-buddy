...
serve(async (req)=>{
  ...
  try {
    ...
    logStep("Capture request received", {
      orderId
    });
    ...
    logStep("PayPal payment captured", {
      orderId,
      status: captureData.status
    });
    // Update bestowal status
    const { error: bestowError } = await supabaseClient.from("bestowals").update({
    ...
    const { error: transactionError } = await supabaseClient.from("payment_transactions").update({
      status: "completed",
    ...
    logStep("ERROR in PayPal capture", {
