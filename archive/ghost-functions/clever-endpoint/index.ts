...
async function verifyBinanceWebhookSignature(rawBody, headers, apiSecret) {
  ...
  if (Number.isFinite(requestTime) && Math.abs(now - requestTime) > toleranceMs) {
    console.warn(`Binance Pay webhook timestamp outside tolerance: now=${now}, timestamp=${requestTime}`);
    return false;
  }
  // Verify signature
...
const handler = async (req)=>{
  ...
  const isValidSignature = await verifyBinanceWebhookSignature(rawBody, req.headers, binanceApiSecret);
  if (!isValidSignature) {
  ...
  const payloadHash = await crypto.subtle.digest("SHA-256", new TextEncoder().encode(rawBody)).then((hash)=>Array.from(new Uint8Array(hash)).map((b)=>b.toString(16).padStart(2, '0')).join(''));
  ...
  if (alreadyProcessed) {
    console.log("Webhook already processed, returning success (idempotent)", {
      webhookId
    });
  ...
  try {
    const { data: bestowal, error: bestowalError } = await supabase.from("bestowals").select(`
          id,
          payment_status,
          payment_reference,
          distribution_data,
        ...
        `).eq("id", merchantTradeNo).single();
    if (bestowalError || !bestowal) {
      console.error("Bestowal not found for webhook:", merchantTradeNo, bestowalError);
      return jsonResponse({
        code: "ERROR",
        message: "Bestowal not found"
      }, 404, req);
    }
    if (status === "SUCCESS" || status === "PAY_SUCCESS") {
      ...
      await supabase.from("bestowals").update({
        ...
        payment_reference: data?.transactionId ?? data?.prepayId ?? bestowal.payment_reference
      }).eq("id", bestowal.id);
      // Deduct balance from bestower's wallet when they send payment
      try {
        const { data: bestowerWallet } = await supabase.from("user_wallets").select("wallet_address").eq("user_id", bestowal.bestower_id).eq("wallet_type", "binance_pay").eq("is_active", true).order("is_primary", {
          ascending: false
        }).limit(1).maybeSingle();
        if (bestowerWallet?.wallet_address) {
          const { data: currentBalance } = await supabase.from("wallet_balances").select("usdc_balance").eq("user_id", bestowal.bestower_id).eq("wallet_address", bestowerWallet.wallet_address).maybeSingle();
      ...
      try {
        ...
        }).eq("bestowal_id", bestowal.id).eq("payment_method", "binance_pay");
      } catch (transactionError) {
        console.warn("Failed to update payment transaction for bestowal:", bestowal.id, transactionError);
      }
      const distribution = bestowal.distribution_data;
      ...
      if (distributionMode === "automatic" && bestowal.payment_status !== "distributed") {
        if (!distribution) {
          console.error("Distribution data missing for bestowal:", bestowal.id);
          return jsonResponse(successResponse, 200, req);
        }
        ...
        try {
          ...
          if (distributionResponse.ok) {
            console.log("Distribution completed for bestowal:", bestowal.id);
          } else {
            console.warn("Distribution function not available or failed. Bestowal will require manual distribution:", bestowal.id);
            // Mark for manual distribution
            await supabase.from("bestowals").update({
              payment_status: "pending_distribution"
            }).eq("id", bestowal.id);
          }
        } catch (distributionError) {
          console.warn("Distribution service unavailable. Bestowal marked for manual distribution:", bestowal.id, distributionError);
        ...
        console.log("Bestowal requires manual distribution. Holding funds in s2gholding:", bestowal.id);
      }
      // Audit log
      await supabase.rpc('log_payment_audit', {
        user_id_param: bestowal.bestower_id,
    ...
    if (status === "PAY_CLOSED" || status === "PAY_FAIL" || status === "FAILED") {
      ...
      try {
        ...
        console.warn("Failed to update payment transaction (failure path):", bestowal.id, transactionFailError);
      }
      return jsonResponse(successResponse, 200, req);
    }
    console.log("Unhandled Binance Pay webhook status:", status, "payload:", payload);
    return jsonResponse(successResponse, 200, req);
  } catch (error) {
...
async function sendBestowalProofMessage(supabase, params) {
  try {
    if (params.distributionData?.proof_sent_at) {
      console.log("Bestowal proof already sent. Skipping:", params.bestowalId);
      return;
    }
    const { data: gosatUser, error: gosatError } = await supabase.from("user_roles").select("user_id").eq("role", "gosat").order("created_at", {
      ascending: true
    }).limit(1).maybeSingle();
    ...
    const summaryLines = [
      ...
      `Distribution: ${params.distributionMode === "manual" ? "Waiting for Gosat release from s2gholding." : "Automatically distributed to recipients."}`,
    ...
    if (params.distributionData) {
      ...
      await supabase.from("bestowals").update({
        distribution_data: updatedDistribution
...
async function sendSowerThankYouMessage(supabase, params) {
  try {
    // Get sower (orchard owner)
    const { data: orchard, error: orchardError } = await supabase.from("orchards").select("user_id, title").eq("id", params.orchardId).single();
    ...
    const { data: sowerProfile } = await supabase.from("profiles").select("first_name, display_name").eq("user_id", sowerId).single();
    const { data: bestowerProfile } = await supabase.from("profiles").select("first_name, display_name").eq("user_id", params.bestowerId).single();
...
async function sendSowerBestowalNotification(supabase, params) {
  try {
    // Get sower (orchard owner)
    const { data: orchard, error: orchardError } = await supabase.from("orchards").select("user_id, title").eq("id", params.orchardId).single();
    if (orchardError || !orchard?.user_id) {
