...
**Devnet cancel/refund proof PASSED 2026-09-06 13:28 UTC.** Cluster flipped to devnet by the owner (13:2x). `scripts/studio/phase-c-devnet-cancel.sql` (as gosat, commits): `orchard_cancel('55f4e02e-...')` -> cancelled true, state cancelling, 1 refund queued (`27fc8591-8805-4f30-bd36-4027485215f8`), 10.00 to return. Worker fired through `invoke_money_job('orchard-refund-worker', {"orchardId": ...})` (request 106820): answered `ok, cluster devnet, claimed 1, result confirmed` 17 seconds later with devnet signature `2iuoASMsL5sW82vnAMu1jMexZPA9pe8mdd9EAoyG7rxH4PAhPc9efNKtZtkTkq1uzXZ2KP7V9AZVvt9QLy5diHva`. On chain (finalized, err null, fee 5000 lamports): hot wallet `6zbpF3HQ...` -> payer ATA `HEZvrLGL...`, 10 USDC; payer `EbSUvuE8...` 13.05 -> 23.05, hot wallet devnet ATA 38.00 -> 28.00. Database: refund row confirmed with the signature, fee_cost 0.01, attempts 0; holding `2df2ff33` refunded; bestowal `ef43a986` payout_status refunded; one `refund_cost` row -0.01 devnet/solana, source `orchard_refunds`; events refund_queued, cancelled, refund_confirmed, cancel_settled; orchard `funding_state = cancelled` with reason; notifications: sower (B) cancelled, bestower (A) pending then Refunded. Liability views (through `liability_snapshot`, PostgREST-equivalent path not yet exercised): devnet held-for-orchards 10.00 -> 0, S2G devnet own 2.20 -> 2.19; live view untouched (liabilities 10.00, operating 2.70). `owed_payout_balances` for B unchanged at 8.70 (the Phase B devnet release). **C2 is done. C3 built below.** The cluster must be flipped back to mainnet-beta by the owner after this proof.

**C3 built 2026-09-06 (screens only, no money logic).** Gosat orchard console at **`/admin/orchards`** (`src/pages/GosatOrchardsPage.tsx`, route gated gosat|admin like /admin/payouts, linked from the Gosat panel): every orchard that holds or held money with state badge (open / fully funded / funded & released / cancelling / cancelled), held total, pockets, sower, age; Cancel (only open|funded, else a disabled button with the refusal reason) opens a dialog with the sentence "This refunds N bestowers a total of $X on their original rails (…)", an unknown-payer warning, a mandatory reason and the orchard title typed to confirm, then calls `orchard_cancel`; a refund-progress card per cancelling/cancelled orchard (per bestower: paid, rail, masked destination, state, attempts, reference with explorer link, last error) with Retry (`orchard_refund_retry`), Write off with a mandatory reason (`orchard_refund_write_off`), Set payer address for unknown payers (`orchard_set_payer_manual`), and Run worker now (`orchard-refund-worker` with the orchard id). Bestower side: `src/lib/orchards/refundLabels.ts` (pure) + `pocketStatus.ts` (own holdings/refunds via RLS) render "Held for this orchard" / "Funded — released to the sower" / "Refund on its way" / "Refunded to the wallet you paid from · tx …" (explorer link, devnet-aware) / "Refund could not be sent — please contact Sow2Grow" on My Seeds (`pocket-state`) and in a "Your pockets in this orchard" card on the orchard page (`my-pocket-state`). Sower side: "Cancelled" / "Cancelling — refunds in progress" badge on the orchard page, the reason shown to the sower only, the funding header says cancelled, and `OrchardPaymentWidget` shows "This orchard was cancelled…" with no bestow button. Treasury: the C2 `refunding` sub-block renders under held-for-orchards plus an alert linking to the console; the orchard list shows funding_state and refunding amounts. Tests: `src/test/orchard-refund-labels.test.ts` (9); Playwright `tests/payments/orchard-console.spec.ts` (3: gosat console states + refusal + confirmed refund with the devnet signature; A sees "Refunded … tx 2iuoASMs…iHva" on the orchard page and My Seeds; B sees the badge and reason) using the owner's gosat account from the gitignored `.env.test`; the Phase A spec now skips cancelled orchards. Not built: Uplift-specific cancel rules (Phase D), "Run worker now" is a manual trigger only (cron still every 10 min).
