// One-off: re-render banner-06-wandering-whisperer at 12s so VO finishes cleanly.
...
const ID = "banner-06-wandering-whisperer";
const VO_SLUG = "06-wandering-whisperer";
const VO = path.resolve(__dirname, `../public/voiceovers/${VO_SLUG}.mp3`);
const OUT = path.resolve(__dirname, "../../public/videos/banners/wandering-whisperer.mp4");
...
execSync(
  `ffmpeg -y -i "${SILENT}" -i "${VO}" -filter_complex "[1:a]apad[a]" -map 0:v -map "[a]" -c:v copy -c:a aac -b:a 192k -t 12 "${OUT}"`,
  { stdio: "inherit" },
);
