...
3. S2G's share: one `revenue_ledger` entry per orchard for `sum(s2g_amount)`, kind `orchard_fee`, referencing the release row. This is the first moment the 15% counts as S2G's. **Settled 2026-09-06:** the ledger exists (Bookkeeping Phase 1, `BOOKKEEPING-PLAN.md`); `orchard_release()` calls `public.record_revenue('orchard_fee', sum(s2g_amount), <environment>, 'orchard_releases', <release id>, <rail>, <release id>::text, now(), <note>)` inside its transaction. `record_revenue` already knows the `orchard_releases` source and refuses it until the table exists, so Phase B only has to create the table and make the call. Environment comes from the holdings' bestowals (`payment_environment(provider, 'orchard', bestowal_id)`); a release mixing live and devnet holdings must record two rows.
...
**Uplift cancel rule (owner decision 2026-09-07, Phase D).** An Uplift can be cancelled exactly like a Launch while it is `open` or `funded` and **no row exists in `orchard_release_payments`** for it. The first party row (written at status `sending` in the same transaction as the ledger release, before any USDC leaves) makes cancel impossible for good: `orchard_cancel()` returns `uplift_parties_already_paid` with a plain message naming the row count and the amount paid, and the console shows that message on the disabled button. From then on the orchard can only be finished with further release payments; a party that cannot be paid goes to `needs_human` (Phase C's state, same gosat alert), where a gosat retries it or voids it and adds a corrected party. A released Uplift whose rows were all voided still cannot be cancelled (`uplift_released`).
...
**Live as of 2026-09-05 evening.** Migration A1 applied server-side (`npx supabase db query --linked -f`), proof row clean; guard tests `scripts/studio/phase-a-rpc-guard-tests.sql` 9/9 pass (rolled back); all 14 functions deployed; `tests/payments/orchard-holdings.spec.ts` passes against the live backend and created the "Phase A test orchard" `55f4e02e-32fe-4013-aa7b-4eff6da77d37` (sower = test account A, 10 x 10 USDC, physical). **Devnet pocket test PASSED 2026-09-06 07:21 UTC** (sower reassigned to test account B because the order function requires a sower payout method; bestower A, Phantom on devnet): signature `eFkR7VNuTemE3vJiVRyriryJJZuMrXR7VQKEXu96cHnhrXCGGLM5gPZvA34vJSPj61KtV5pnhmffiDiDiKixzZW`, hot wallet +10,010,000 raw (10.00 pocket + 0.01 flat Solana fee); holding `2df2ff33-6aff-455a-bfb5-d219264193f5` held / hot_wallet / solana, gross 10.00 sower 8.70 s2g 1.30 fee 0.01, delivery address and signature stored; bestowal `ef43a986…` completed + held_for_orchard; sower ledger 0 → 0; filled_pockets 0 → 1; funding 0/100 → 10/100, 1/10, not funded; treasury held-for-orchards 0 → 10.00. Proof queries: `scripts/studio/phase-a-devnet-before.sql` / `-after.sql`. `payer_address` on the holding is null (Phase C backfill, as designed). Cluster flipped back to mainnet-beta 07:25 UTC.
...
**Built, applied, and proven on devnet 2026-09-06 11:55 UTC.** One-pocket orchard `9d8fbab2…` sown by B, one 10.01 USDC devnet pocket by A (signature `b4NquLfz…z387Jhn`, hot wallet +10,010,000 raw): `holding_created`, `funded` and `released` events in the same second; holding released; bestowal `pending` again; B owed 8.70 via `owed_payout_balances`; one `orchard_fee` row 1.30 devnet/solana with `release_ref` = orchard id; release row 8.70 / 1.30 / gift 0 / auto; 2 notifications; devnet liability view: owed 0 → 8.70, S2G own 0.90 → 2.20, live view untouched. Known gap for Phase C: a stale intent paid after release would still create a `held` holding on a released orchard (`orchard_apply_holding` does not yet refuse or flag it). Migration `supabase/migrations/20260906190000_orchard_release.sql` (applied server-side): `orchards.orchard_kind` (launch, default) / `funding_state` (open → funded → released, cancelled reserved for Phase C) / `funded_at` / `released_at`; `orchard_releases` (one per orchard, totals at release, trigger auto|gosat); `orchard_stock` (gift units); `orchard_release(orchard_id)` public entry (service role or gosat/admin, idempotent) over `orchard_release_locked()`; `orchard_release_if_funded()` called at the end of `orchard_apply_holding()`, so the holding that completes the funding releases the orchard in the same transaction under the orchard row lock. `orchard_funding_status()` gains `funding_state`, `released`, `released_at`.
...
### Phase D status (2026-09-07)

**Built, applied, fixture-proven; devnet proof pending the owner's pocket.** Migration `supabase/migrations/20260907090000_orchard_uplift.sql` applied server-side (`npx supabase db query --linked -f`), proof row all true.

Schema: `orchards.opened_by_gosat`; `orchard_release_payments` (label, amount, rail, destination, reference, status sending|paid|failed|needs_human|voided, environment, attempts, paid_at, paid_by_gosat); `trg_orchards_uplift_gate` (only a gosat/admin or the service role may insert kind `uplift` or change a kind, and only before any pocket exists; a member gets `uplift_orchards_are_gosat_only`, SQLSTATE 42501 → HTTP 403); `trg_orchard_release_payments_guard` (sum of non-voided rows per orchard ≤ that orchard's `orchard_releases.sower_total`, refused as `party_payments_exceed_sower_total`; a row on an unreleased orchard is `orchard_not_released`); `orchard_parties_paid_v` (the tribe's read-only list: label, amount, rail, paid_at of paid rows; no destination, no reference).

Functions: `orchard_release_if_funded()` now stops an Uplift at `funded` and writes one in-app `user_notifications` row per gosat/admin (`orchard_uplift_funded`, S2G has no email) instead of releasing; `orchard_uplift_fund_now(orchard, note)` (gosat, open → funded, note logged as a `funded_override` event; the parties can still only be paid from what is actually held on-app); `orchard_uplift_release(orchard, parties, retry_ids, actor)` (gosat/admin or service role: validates every party — label, positive 2-decimal amount, Solana-shaped destination, USDC only — checks the sum against the sower total and the environment **before** anything changes, performs the ledger release on the first call through `orchard_release_uplift_locked()` — holdings → released, bestowal rows → `paid_to_parties` so `owed_payout_balances()` never counts them, `orchard_fee` via `record_revenue` per environment exactly as Launch, no sower Books income, notifications — then inserts one `sending` row per party and re-arms retried rows; a second call only adds or retries); the worker seam `orchard_uplift_payment_paid/fail/defer/needs_human` (service role; paid is idempotent per reference and refuses a second reference; the 3rd failure parks the row at `needs_human` and alerts gosats); `orchard_uplift_payment_void` (gosat; an unsent failed/parked row → voided, freeing the amount); `orchard_cancel()` gains the Uplift rule (section 5); `liability_snapshot()` gains `held_for_orchards.uplift_unpaid` (released Uplift sower money not yet paid to parties, counted in `liabilities_total`).

Edge functions: new `orchard-release-uplift` (gosat session or service apikey; body `{orchardId, parties[], retryPaymentIds[]}`; per row: the money-direction guardrail in `_shared/orchardUpliftRules.ts` on the fresh row — status sending, no reference, USDC, environment = the function's cluster, per-transaction cap 50 → needs_human for the Squad, daily cap 200 → defer — then hot-wallet pre-flight, a chain search for a prior send on retries, `sendUsdcPayout` at finalized commitment, `orchard_uplift_payment_paid(signature)`; partial failure leaves paid rows paid and returns which rows failed). `create-orchard-bestowal-order` refuses any orchard whose `funding_state` is not `open` (`orchard_not_open`) and skips the sower payout-method and settlement-consent gates for an Uplift (there is no sower payout). `config.toml` now carries `verify_jwt = false` for `orchard-refund-worker` (live value) and `orchard-release-uplift`.

Screens: `CreateOrchardPage` offers Launch / Uplift only to a gosat/admin (the trigger is the gate); the orchard page shows "Uplift orchard: Sow2Grow pays the parties directly" and, after release, "Where the gifts went" from the view; the bestow widget's copy and My Seeds' pocket state say "Sow2Grow pays the parties directly" for an Uplift; `/admin/orchards` shows the Uplift badge, Fund now (note + typed title), a release card per funded/released Uplift (pool, committed, paid, left to pay; party rows with state, attempts, reference link, last error, Retry / Void; the party form with client-side validation, typed-title confirmation, USDC only) and the Uplift cancel refusal.

Proofs: `scripts/studio/phase-d-uplift-tests.sql` 22/22 (rolled back: gate 42501 for a member on insert and on a kind flip; funded-not-released with gosat notifications; a funded Uplift with no rows still cancels; over-total refused before releasing; release to two parties with the fee row, `paid_to_parties`, nothing owed; cancel refused afterwards; DB guard on one extra cent; paid idempotent; three failures → needs_human; void, corrected party, settled event, bestower notification; view columns; `uplift_unpaid` 17.40 → 0; fund-now refused for a member, logged for a gosat, release with nothing held refused). Regression: Phase A guard 9/9, Phase B release 10/10 (its owed assertions made baseline-relative, since B has real owed rows since the devnet release), Phase C refund 27/27. Unit: `src/test/orchard-uplift.test.ts` (19: party validation, guardrail, caps, cancel and creation mirrors, client/server drift) — orchard suites 48/48; `platform-fee-drift` green. Playwright `tests/payments/orchard-uplift.spec.ts` part 1 (gate refuses A, the gosat's Uplift test orchard, console badge + Fund now, the member-facing wording); part 2 (`PHASE_D_RELEASE=1`) releases to two parties and checks both signatures and the tribe-facing list — runs only after the devnet pocket.

Devnet proof target: the Playwright part 1 run created the real **"Phase D uplift test orchard" `e476b50a-1933-48f4-8e15-e4c1513a5317`** (kind uplift, opened by the gosat `04754d57…`, 1 pocket × 10 USDC, digital, open). BEFORE snapshot (`scripts/studio/phase-d-devnet-proof.sql`, 2026-09-07): 0 holdings, 0 party rows, 0 gosat notifications, devnet liabilities 14.65 with `uplift_unpaid` 0, live `uplift_unpaid` 0. Steps: owner flips the cluster to devnet, A buys the one pocket with Phantom on devnet → the orchard reads funded and every gosat gets the in-app notification; then `PHASE_D_RELEASE=1` Playwright (or the console by hand) releases it to two parties; the AFTER snapshot must show the release row 8.70 / 1.30, one devnet `orchard_fee` 1.30, two paid rows with signatures, `uplift_unpaid` 0 once both are paid, nothing owed to the opener; flip back to mainnet-beta.

Open: the devnet proof itself; PayPal party payments (section 9, item 6); `liability_snapshot.other_environments` does not yet break out `uplift_unpaid` per test environment (the live figure and the per-environment `uplift_unpaid` object do).
...
4. ~~**Where the revenue entry lives.**~~ Settled 2026-09-06: `revenue_ledger` + `record_revenue()` are live (Bookkeeping Phase 1). Phase B writes `orchard_fee` through it; see section 4 step 3.
5. ~~**Two non-admin test accounts.**~~ Settled: A, B and the owner's gosat login live in the gitignored `.env.test` (2026-09-06).
6. **PayPal party payments (Phase D, owner decision 2026-09-07).** Party payments are USDC only until PayPal approves the app. `orchard_release_payments.rail` already allows `paypal` and the row shape is rail-neutral, but `orchard_uplift_release()` refuses any non-Solana party (`paypal_party_payments_not_yet_available`), the console offers only a Solana wallet field, and `orchard-release-uplift` has no PayPal branch. When PayPal approves: add a PayPal Payouts branch in the function (same primitive the payout run would use), accept `rail = 'paypal'` with an email destination in the RPC, extend the guardrail's environment check to the PayPal sandbox/live pair, and cover it in the fixture.
7. **Two distinct devnet destinations for the Phase D proof.** Both test accounts resolve to the same devnet wallet (B's payout address and A's Phantom are the owner's `EbSUvuE8…`). The proof therefore pays two parties to that one wallet unless the owner supplies a second devnet address through `PHASE_D_PARTY2_ADDRESS`; two rows and two signatures are proven either way.
