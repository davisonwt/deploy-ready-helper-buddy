        ...
        <Card className="bg-white/95 backdrop-blur-lg border-0 shadow-2xl hover:shadow-3xl transition-all duration-500 overflow-hidden">
          ...
          <CardContent className="p-8">
            <form onSubmit={handleSubmit} className="space-y-6">
              ...
              {/* Registering as a business — optional tier selector */}
              <div className="space-y-3 p-4 rounded-xl border-2 border-green-100 bg-green-50/40">
                <div>
                  <label className="text-sm font-semibold text-green-800">
                    Are you registering a business? <span className="text-green-600 font-normal">(Optional)</span>
                  </label>
                  <p className="text-xs text-gray-600 mt-1">
                    Pick the option that best describes your size. You can change this later in Seller Business Settings.
                  </p>
                </div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                  <button
                    type="button"
                    onClick={() => setFormData({ ...formData, businessTier: "" })}
                    className={`text-left rounded-lg border-2 p-3 transition-all ${
                      formData.businessTier === ""
                        ? "border-green-500 bg-white shadow"
                        : "border-gray-200 bg-white/70 hover:border-green-300"
                    }`}
                  >
                    <div className="font-semibold text-sm text-gray-800">👤 Personal sower</div>
                    <div className="text-xs text-gray-500 mt-0.5">Just me — not a business</div>
                  </button>
                  ...
                  {TIERS.map((t) => (
                    <button
                      key={t.id}
                      type="button"
                      onClick={() => setFormData({ ...formData, businessTier: t.id })}
                      className={`text-left rounded-lg border-2 p-3 transition-all ${
                        formData.businessTier === t.id
                          ? "border-green-500 bg-white shadow"
                          : "border-gray-200 bg-white/70 hover:border-green-300"
                      }`}
                      title={t.explainer}
                ...
                {formData.businessTier && (
                  <p className="text-xs text-green-700 bg-white/70 border border-green-200 rounded p-2">
                    {TIERS.find((t) => t.id === formData.businessTier)?.explainer}
                  </p>
                )}
              </div>
