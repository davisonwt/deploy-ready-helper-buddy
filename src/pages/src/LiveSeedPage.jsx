        ...
        <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }}
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
            {SESSION_TYPES.map(type => (
              <motion.button key={type.value} whileHover={{ scale: 1.03, y: -2 }} whileTap={{ scale: 0.97 }}
          ...
          <div style={{ padding: 16, display: 'flex', flexDirection: 'column', gap: 16, flex: 1 }}>
            ...
            <Link to={`/animated-orchard/${orchardId}`} style={{ textDecoration: 'none' }}>
              <motion.button whileHover={{ scale: 1.03 }} whileTap={{ scale: 0.97 }}
                style={{ width: '100%', padding: '16px 0', background: `linear-gradient(135deg, ${typeConfig.color}, ${typeConfig.color}99)`, border: 'none', borderRadius: 14, color: '#fff', fontWeight: 800, fontSize: 16, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8, boxShadow: `0 8px 30px ${typeConfig.color}50` }}>
                <Heart style={{ width: 20, height: 20 }} />
                Bestow from ${pocketPrice}
              </motion.button>
            </Link>
            <button onClick={handleShare}
              style={{ width: '100%', padding: '12px 0', background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.08)', borderRadius: 12, color: '#64748b', fontWeight: 600, fontSize: 13, cursor: 'pointer' }}>
              📤 Share this live session
            </button>
            <div style={{ fontSize: 11, color: '#374151', textAlign: 'center', paddingBottom: 8 }}>S2G Live · Powered by sow2growapp.com</div>
          </div>
        </div>
      </div>
    </div>
