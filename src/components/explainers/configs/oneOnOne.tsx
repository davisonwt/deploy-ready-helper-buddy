  ...
  return (
    <motion.div {...sceneAnim} className="text-center">
      <motion.p
        ...
        className="text-xs uppercase tracking-[0.3em] mb-3" style={{ color: EMBER }}
      >
        How it works
      </motion.p>
      <h2 className="text-4xl sm:text-6xl mb-3" style={{ fontFamily: '"Fraunces", serif', fontWeight: 500, color: TEXT }}>
        1-on-1 Live
      </h2>
  ...
  return (
    <motion.div {...sceneAnim} className="w-full max-w-md">
      <div className="rounded-xl border p-5" style={{ background: `${PANEL}66`, borderColor: `${TEAL}33` }}>
        <p className="text-xs uppercase tracking-wider mb-4" style={{ color: MUTED }}>Communications Hub</p>
        <motion.button
        ...
        <p className="text-xs mt-4 text-center" style={{ color: MUTED }}>Tap to open the launcher.</p>
      </div>
    </motion.div>
  );
}
  ...
  return (
    <motion.div {...sceneAnim} className="w-full max-w-md">
      <div className="rounded-xl border p-4" style={{ background: `${PANEL}66`, borderColor: `${TEAL}33` }}>
        <p className="text-xs uppercase tracking-wider mb-3" style={{ color: MUTED }}>Invite tribe member</p>
        <div className="space-y-2">
          {members.map((m, i) => (
            <motion.div key={m.name}
              initial={{ opacity: 0, x: -8 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: i * 0.15 }}
              className="flex items-center gap-3 p-2.5 rounded-lg border"
              style={{ background: m.selected ? `${TEAL}1A` : 'transparent', borderColor: m.selected ? `${TEAL}66` : `${TEAL}1A` }}
            >
              <div className="w-9 h-9 rounded-full flex items-center justify-center font-semibold"
                style={{ background: `${TEAL}33`, color: TEXT }}>{m.initial}</div>
              ...
              {m.selected && (
                <motion.div initial={{ scale: 0 }} animate={{ scale: 1 }} transition={{ delay: 0.8, type: 'spring' }}
                  className="w-6 h-6 rounded-full flex items-center justify-center" style={{ background: TEAL }}>
      ...
      <div className="rounded-xl border overflow-hidden" style={{ background: `${PANEL}66`, borderColor: `${TEAL}33` }}>
        ...
        <div className="p-4 space-y-2.5 min-h-[180px]">
          <motion.div initial={{ opacity: 0, y: 6 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }}
            className="max-w-[75%] px-3 py-2 rounded-2xl rounded-tl-sm text-sm" style={{ background: `${TEAL}1A`, color: TEXT }}>
            Hey, ready to talk?
          </motion.div>
          <motion.div initial={{ opacity: 0, y: 6 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.9 }}
            ...
            <div className="flex items-end gap-0.5 h-4">
              {[6, 12, 8, 14, 10, 7, 11].map((h, i) => (
                <motion.div key={i} initial={{ height: 2 }} animate={{ height: h }} transition={{ delay: 1 + i * 0.05 }}
          ...
          <motion.div initial={{ opacity: 0, y: 6 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 1.8 }}
            className="max-w-[55%] rounded-xl overflow-hidden border" style={{ borderColor: `${TEAL}33` }}>
        ...
        <div className="px-4 py-3 border-t flex items-center gap-2" style={{ borderColor: `${TEAL}1A`, background: `${BG}80` }}>
          <div className="flex-1 px-3 py-1.5 rounded-full text-xs" style={{ background: `${TEAL}14`, color: MUTED }}>Message…</div>
      ...
      <div className="rounded-xl border p-6" style={{ background: `${PANEL}66`, borderColor: `${TEAL}33` }}>
        <div className="flex items-center justify-center gap-3 mb-5">
          ...
          <motion.div animate={{ opacity: [0.3, 1, 0.3] }} transition={{ duration: 1.4, repeat: Infinity }}
            className="text-xs uppercase tracking-wider" style={{ color: TEAL }}>connecting</motion.div>
        ...
        <p className="text-xs mt-4" style={{ color: MUTED }}>Tap the call button for live voice or video.</p>
      </div>
    </motion.div>
  );
}
  ...
  return (
    <div className="flex flex-col items-center gap-1">
      <div className="w-12 h-12 rounded-full flex items-center justify-center font-semibold"
        style={{ background: `${color}33`, color: TEXT, border: `2px solid ${color}` }}>{label[0]}</div>
...
function OutroScene() {
  return (
    <motion.div {...sceneAnim} className="text-center">
      <h3 className="text-3xl sm:text-4xl mb-3" style={{ fontFamily: '"Fraunces", serif', fontWeight: 500, color: TEXT }}>
        Private. Secure.
      </h3>
      <p className="text-base" style={{ color: EMBER, fontFamily: '"Fraunces", serif' }}>Built for real connection.</p>
    </motion.div>
  );
}
...
export const oneOnOneConfig: ExplainerConfig = {
  ...
  outroTitle: 'Private. Secure.',
  outroTagline: 'Built for real connection.',
  theme: { bg: BG, panel: PANEL, accent: TEAL, ember: EMBER, muted: MUTED, text: TEXT, font: '"Fraunces", serif', fontWeight: 500 },
  segments: [
    { id: 'intro', weight: 5.5, Scene: IntroScene },
    { id: 'step1', weight: 5.5, Scene: Step1Scene },
    { id: 'step2', weight: 5.5, Scene: Step2Scene },
    { id: 'step3', weight: 7.5, Scene: Step3Scene },
    { id: 'step4', weight: 6,   Scene: Step4Scene },
    { id: 'outro', weight: 4,   Scene: OutroScene },
  ],
};
