...
import { Check, X, Clock, User, MapPin, Phone, Mail, DollarSign, Wrench } from 'lucide-react';
...
export function ServiceProviderApplicationsDashboard() {
  ...
  const fetchApplications = async () => {
    ...
    if (error) {
      ...
      toast.error('Failed to load service provider applications');
    } else {
  ...
  const updateStatus = async (id: string, newStatus: 'approved' | 'rejected') => {
    ...
    toast.success(`Service provider ${newStatus}`);
    fetchApplications();
  };
  ...
  return (
    <div className="space-y-4">
      ...
      {loading ? (
        <div className="text-center py-8 text-muted-foreground">Loading service provider applications...</div>
      ) : applications.length === 0 ? (
        <div className="text-center py-8 text-muted-foreground">No {filter !== 'all' ? filter : ''} service provider applications found.</div>
      ) : (
            ...
            <Card key={app.id} className="border">
              ...
              <CardContent className="space-y-3">
                ...
                {app.service_description && (
                  <p className="text-sm text-muted-foreground bg-muted/50 rounded-lg p-3">{app.service_description}</p>
                )}
