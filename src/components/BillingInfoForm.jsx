  ...
  const loadBillingInfo = async () => {
    ...
    try {
      const { data: profile, error } = await supabase
        .from('profiles')
        .select('billing_address_line1, billing_address_line2, billing_city, billing_state, billing_postal_code, billing_country, billing_phone, billing_email, billing_organization')
        .eq('user_id', user.id)
        .single()
