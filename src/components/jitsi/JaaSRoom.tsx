...
export default function JaaSRoom({
  roomName,
  displayName = 'Guest',
  userEmail = '',
  jwt,
  ...
  useEffect(() => {
    const loadJaaSScript = () => {
      // Check if JaaS API is already loaded
      ...
      script.async = true;
      script.onload = initializeJaaS;
      script.onerror = () => {
        toast({
          title: 'Error',
          description: 'Failed to load JaaS (8x8). Please check your internet connection.',
          variant: 'destructive',
        });
        setIsLoading(false);
      };
    ...
    const initializeJaaS = () => {
      ...
      try {
        const options: any = {
          roomName: JAAS_CONFIG.getRoomName(roomName),
          width: '100%',
          height: '100%',
        ...
        const activeJwt = jwt || JAAS_CONFIG.jwt;
        if (activeJwt) {
          options.jwt = activeJwt;
        }
        ...
        jitsiApi.current.addListener('videoConferenceJoined', () => {
          ...
          toast({
            title: 'Connected',
            description: 'You joined the JaaS room',
          });
        });
        ...
        console.error('Error initializing JaaS:', error);
        toast({
          title: 'Error',
          description: 'Failed to initialize JaaS video room',
          variant: 'destructive',
        });
        setIsLoading(false);
      }
  ...
  }, [roomName, displayName, userEmail, jwt, toast]);
      ...
      {isLoading && (
        <div className="absolute inset-0 flex items-center justify-center bg-background z-10">
          <Card className="p-8 text-center">
            ...
            <p className="text-lg">Connecting to JaaS room...</p>
          </Card>
        </div>
      )}
