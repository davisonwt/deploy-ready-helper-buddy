...
/**
 * The old /my-tribe page's own content (invitation code, share buttons,
 * tribe size, direct-referral list) -- moved here wholesale so it stays
 * reachable from the new walk-through's "Invite" hotspot sheet
 * (src/pages/MyTribePage.tsx), per the explicit "keep everything the old
 * page showed reachable inside those sheets" instruction. Logic
 * unchanged from the page this was extracted from -- just the outer
 * page chrome (background, "Go Back" link) dropped, since the sheet
 * wrapper now owns that.
 */
export default function TribeInviteSheetContent() {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="rounded-2xl border border-cyan-400/25 bg-[#0f172a]/80 backdrop-blur p-5 shadow-[0_0_30px_rgba(34,211,238,0.10)] max-w-sm">
        <div className="flex items-center justify-between">
          <span className="text-xs uppercase tracking-wider text-slate-400">Tribe size</span>
          <Users className="h-4 w-4 opacity-70 text-cyan-200" />
        </div>
        <div className="text-3xl font-extrabold text-white mt-2">{stats.total}</div>
        <p className="text-xs text-slate-400 mt-1">{stats.completed} active</p>
      </div>
