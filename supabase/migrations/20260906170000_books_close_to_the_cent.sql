-- Closing the books to the cent (2026-09-06, owner's request):
--   1. treasury_movements.occurred_at (when the money actually moved; created_at
--      stays the record time) and record_treasury_movement(_occurred_at).
--   2. record_revenue_correction(_rail): a correction can carry the rail the
--      money sits on, so the per-wallet view places it.
--   3. liability_snapshot(): parked S2G Balance split by the rail it came in on
--      (held_for_members.parked.by_rail), so PayPal's expectation includes the
--      parked PayPal-sale money it physically holds.
-- Prefer: npx supabase db query --linked -f <this file>

ALTER TABLE public.treasury_movements ADD COLUMN IF NOT EXISTS occurred_at timestamptz NOT NULL DEFAULT now();
UPDATE public.treasury_movements SET occurred_at = created_at WHERE occurred_at = created_at OR occurred_at IS NULL;
...
BEGIN
  ...
  END IF;
  INSERT INTO public.treasury_movements (kind, wallet, amount_usd, currency, environment, reference, notes, created_by, occurred_at)
  VALUES (_kind, _wallet, round(_amount_usd, 2), COALESCE(_currency, 'USDC'), COALESCE(_environment, 'live'), _reference, _notes, auth.uid(), COALESCE(_occurred_at, now()))
  RETURNING * INTO v_row;
  RETURN v_row;
END;
$cl_1$;
DROP FUNCTION IF EXISTS public.record_treasury_movement(text, text, numeric, text, text, text, text);
REVOKE ALL ON FUNCTION public.record_treasury_movement(text, text, numeric, text, text, text, text, timestamptz) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION public.record_treasury_movement(text, text, numeric, text, text, text, text, timestamptz) TO authenticated, service_role;
...
BEGIN
  ...
  END IF;
  RETURN public.record_revenue('correction', _amount, _environment, NULL, NULL, COALESCE(_rail, 'none'), NULL,
                               _recognised_at, _notes, 'USD', _idempotency_key, auth.uid());
END;
$cl_2$;
DROP FUNCTION IF EXISTS public.record_revenue_correction(numeric, text, text, text, timestamptz);
REVOKE ALL ON FUNCTION public.record_revenue_correction(numeric, text, text, text, timestamptz, text) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION public.record_revenue_correction(numeric, text, text, text, timestamptz, text) TO authenticated, service_role;
...
BEGIN
  ...
  -- Parked S2G Balance ledger, per row environment from its source, and the
  -- rail the money came IN on (it physically sits on that rail).
...
-- Proof (service-role context; the body only ever TRUNCATEs its temp tables).
SET LOCAL ROLE service_role;
SELECT set_config('request.jwt.claims', '{"role":"service_role"}', true);
SELECT public.liability_snapshot('live') -> 'held_for_members' -> 'parked' AS parked_live,
       (SELECT count(*) FROM information_schema.columns WHERE table_name = 'treasury_movements' AND column_name = 'occurred_at') AS occurred_at_column;
