-- Third bug in the same 2026-08-26 incident chain, found only after fixing
-- the previous two: books_sync_product_sale() inserts expenses rows with
-- category = 'Platform fees' / 'Whisperer commission' -- neither of which
-- is in expenses_category_check's allowed list (Software/Travel/Meals/
-- Office/Marketing/Payroll/Other only). This trigger has apparently never
-- once successfully completed an expense insert against a books-enabled
-- sower -- it was always going to hit this the moment the earlier
-- ON CONFLICT bug got fixed. Same fix already applied to
-- _shared/postFinalize/books.ts's own inserts: use 'Other'. The
-- descriptive text ("Sow2Grow platform fee — <title>", "Whisperer
-- commission (X%) — <title>") already carries the real context in
-- `description`; only the constrained `category` column changes.
...
BEGIN
  ...
  IF COALESCE(NEW.s2g_fee,0) > 0 THEN
    INSERT INTO public.expenses (
      ...
      NEW.s2g_fee, COALESCE(v_cur,'USD'), 'Other', 'Sow2Grow',
  ...
  IF NEW.whisperer_id IS NOT NULL THEN
    ...
    IF COALESCE(v_whisper,0) > 0 THEN
      INSERT INTO public.expenses (
        ...
        v_whisper, COALESCE(v_cur,'USD'), 'Other', 'Whisperer',
        COALESCE(NEW.created_at, now())::date, 'marketplace',
