-- Bookkeeping Phase 2 hotfix (2026-09-06): liability_snapshot() failed
-- through PostgREST with "DELETE requires a WHERE clause".
--
-- Root cause: the function reset its two temp tables with a bare
-- `DELETE FROM _ls_owed;` / `DELETE FROM _ls_hold;`. PostgREST's database
-- role (authenticator) is configured with session_preload_libraries =
-- safeupdate, which refuses any DELETE or UPDATE without a WHERE clause --
-- so every call from the edge function (service-role client -> PostgREST)
-- raised, and treasury-balances answered 500 liability_snapshot_failed.
-- The Management API path used to apply and test the migration does not
-- preload safeupdate, which is why the proof and the fixture passed.
--
-- Fix: TRUNCATE instead (safeupdate does not police TRUNCATE). Same body
-- otherwise. Proof: the call succeeds with safeupdate loaded.
--
...
-- Proof. (LOAD 'safeupdate' is not permitted through the Management API; the
-- PostgREST-path proof is scripts/studio/liability-postgrest-probe.sql, run
-- before and after this migration on 2026-09-06.)
SET LOCAL ROLE service_role;
SELECT set_config('request.jwt.claims', '{"role":"service_role"}', true);
SELECT jsonb_build_object(
  'live_liabilities_total', public.liability_snapshot('live') ->> 'liabilities_total',
  'devnet_liabilities_total', public.liability_snapshot('devnet') ->> 'liabilities_total',
  'service_role_context', true
) AS proof;
