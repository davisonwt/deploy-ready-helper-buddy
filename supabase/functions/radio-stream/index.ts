...
import { resolveSlotOverride } from "../_shared/radioSlots.ts";
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // A scheduled Grove Station slot overrides autopilot -- same resolver
    // radio-now-playing consults, so both endpoints always agree on what's
    // live. Falls through to exact-unchanged autopilot once the rundown
    // runs out or no slot is scheduled.
    const override = await resolveSlotOverride(service, now);
    if (override) {
      const remaining = override.segment.durationSeconds - override.segment.offsetSeconds;
      if (override.segment.kind === "song" && override.segment.trackProductId) {
        const { data: product, error: productError } = await service
          .from("products")
          .select("file_url")
          .eq("id", override.segment.trackProductId)
          .maybeSingle();
        const parsed = product?.file_url ? extractBucketAndPath(product.file_url) : null;
        if (!productError && parsed) {
          return await signAndRedirect(service, parsed.bucket, parsed.path, remaining);
        }
        // Referenced product vanished (deleted/unpublished after the
        // rundown was built) -- fall through to autopilot below rather
        // than serve a 500 for the rest of a 2h slot window.
      } else if (override.segment.audioPath) {
        return await signAndRedirect(service, "dj-rundown-segments", override.segment.audioPath, remaining);
      }
    }
...
async function signAndRedirect(service: any, bucket: string, path: string, remainingSeconds: number): Promise<Response> {
  const ttl = Math.min(MAX_TTL_SECONDS, Math.max(MIN_TTL_SECONDS, Math.ceil(remainingSeconds) + TTL_MARGIN_SECONDS));
  const { data: signed, error: signError } = await service.storage.from(bucket).createSignedUrl(path, ttl);
